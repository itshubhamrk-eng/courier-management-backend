...
import {
  WalletTransaction, CreditRequest, SubTransactionType, CREDIT_SUB_TYPES, formatMoney, subTypeLabel
...
const SUB_TYPES: SelectOption[] = CREDIT_SUB_TYPES.map((s) => ({ value: s, label: subTypeLabel(s) }));

/**
 * Manual credit (top-up) of a branch wallet by a COMPANY_ADMIN. Amount + reason (sub type) +
 * optional reference and remarks. Settles immediately; returns the WalletTransaction so the
 * caller refreshes without a second fetch. No mock: posts straight to /credit.
 */
@Component({
  ...
  template: `
    <div class="md">
      <h2 class="text-h2">Credit wallet</h2>
      <p class="text-caption">{{ data.walletName }} · current balance {{ balance() }}</p>
...
export class CreditDialog {
  readonly ref = inject(MatDialogRef<CreditDialog>);
  readonly data = inject<CreditDialogData>(MAT_DIALOG_DATA);
  ...
  protected balance(): string { return formatMoney(this.data.currentBalance, this.data.currency); }
  ...
  save(): void {
    ...
    this.busy.set(true);
    this.service.credit(body).subscribe({
      next: (t: WalletTransaction) => { this.busy.set(false); this.notify.success('Wallet credited.'); this.ref.close(t); },
      error: (e) => { this.busy.set(false); this.notify.error(e?.error?.message ?? 'Could not credit the wallet.'); }
    });
  }
}
