    ...
    public void on(ShipmentEvent.PrepaidBookingConfirmed event) {
        try {
            CompanyContext.runAs(event.companyId(), () -> walletService.debitForBooking(
                    new BookingDebitCommand(event.bookingBranchId(), event.netAmount(),
                            event.shipmentNumber(), "Freight for shipment " + event.shipmentNumber())));
        } catch (RuntimeException e) {
            log.error("Could not debit branch {} for shipment {} ({}); the shipment stays "
                    + "booked, undebited — reconcile manually", event.bookingBranchId(),
