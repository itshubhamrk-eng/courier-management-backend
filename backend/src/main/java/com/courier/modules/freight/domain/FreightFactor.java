 ...
 *
 * <p><b>Both ranges are half-open</b> — {@code [fromKm, toKm)} and
 * {@code [fromWeight, toWeight)} — the same convention {@code rate.domain.Rate}'s weight
 * slab uses. No two <i>active</i> cells may cover the same (distance, weight) point; since
 * this is a 2D grid, a conflict requires <b>both</b> the distance ranges and the weight
 * ranges to overlap (see {@link #overlaps(FreightFactor)}) — two cells can share a distance
 * band as long as their weight bands don't collide, and vice versa.
 */
@Entity
@Getter
@Setter
